...
export default class Command extends CommandBase {
  constructor(client) {
    super(client, {
      ...
      commandAliases: ["rm"],
      commandDescription: "Remova saldo de algum usuário.",
  ...
  async execute(message, args) {
    ...
    await this.client.logger.event(
      `O usuário @${message.author.username} (${
        message.author.id
      }) removeu ${this.client.utils.formatNumberToLocale(amount)} ${
        economy.names.money
      } do saldo do usuário @${creditor.username} (${creditor.id})`,
    );
    await this.client.database.updateUserBalance(creditor.id, -amount);
    await this.client.database.createTransaction({
      source: 1,
      givenBy: creditor.id,
      givenByUsername: creditor.username,
      givenAt: Date.now(),
      transactionAmount: amount,
    });

    return await message.reply({
      content: `✅ ${message.author}, você **removeu** ${
