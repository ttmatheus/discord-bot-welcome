...
export default class Interaction extends InteractionBase {
  ...
  async execute(interaction, options) {
    ...
    const opponent = await this.client.users
      .fetch(interaction.options.data[0].value)
      .catch((err) => null);
    ...
    let opponentData = { userBalance: 0 };

    if (opponent)
      opponentData = await this.client.database.getOrUpdateUser(opponent.id);

    const preValues = [
      { name: "tudo", value: "all" },
      { name: "metade", value: "half" },
      { name: "sobras", value: "sobras" },
      { name: `mínimo`, value: economy.limiters.coinflip.min },
    ...
    if (typeof amount === "number" && amount >= economy.limiters.coinflip.min) {
      values.push({
